...
if($_POST["service_name"] || $_POST["service_price"]) {
    ...
    if (empty($_POST["service_name"]) || check_length($_POST["service_name"], 30)) {
        $msg1 = "Введите корректное название";
    } elseif (!isset($_POST["service_price"]) || check_length($_POST["service_price"], 10) || !is_numeric($_POST["service_price"])) {
        $msg1 = "Введите корректную цену";
    } else {
   ...
   <form action="" method="post">
    <table>
      <tr>
        ...
        <td><input type="text" name="service_name" placeholder="Наименование изделия" class="form-control" value="<?= isset($_GET['red_serviceid']) ? $service['service_name'] : $service_name; ?>"></td>
      </tr>
      <tr>
        <td><input type="text" name="service_price" size="2" placeholder="Стоимость изделия" class="form-control" value="<?= isset($_GET['red_serviceid']) ? $service['service_price'] : $service_price; ?>">
        </td>
